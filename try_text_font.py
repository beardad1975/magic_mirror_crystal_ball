...
白色透明 = color.rgba(255,255,255,0)
白色不透明 = color.rgba(255,255,255,255)
...
登場動畫 = 動畫順序(
    執行(setattr,魔鏡角色,'位置',[0,5,0]),  # 位於上方
    執行(setattr,魔鏡角色,'旋轉',[0,180, 0]),   # 頭向後
    執行(setattr,魔鏡角色,'顏色', 透明), # 透明
    執行(魔鏡角色.顏色動畫,不透明, 持續=1),  # 1秒內轉不透明
    執行(魔鏡角色.位置動畫,[0,0,0], 持續=1),  # 1秒內移至中間
    1,  # 等一秒
    執行(魔鏡角色.旋轉動畫 , [0,0,0],持續=1), # 1秒後轉向前
    1,  # 等一秒
    執行(print, 'end'), # 印出結束
    auto_destroy = False ,
    )
