...
角色到上方 = 動作(setattr,魔鏡角色,'位置',[0,5,0])
角色朝後 = 動作(setattr,魔鏡角色,'旋轉',[0,180, 0])
角色隱藏 = 動作(setattr,魔鏡角色,'顏色', color.clear)
角色顯示動畫 = 動作(魔鏡角色.顏色動畫,color.white, 持續=1)
角色移至中間動畫 = 動作(魔鏡角色.位置動畫,[0,0,0], 持續=1)

角色轉向前動畫 = 動作(魔鏡角色.旋轉動畫 , [0,0,0],持續=1)
